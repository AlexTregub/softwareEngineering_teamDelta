 ...
 */
 ...
 (function(modules) { 
 	...
 	__webpack_require__.n = function(module) {
 		var getter = typeof module !== 'undefined' && module.__esModule ?
...
function loadAudioWorkletModules() {
  return Promise.all(moduleSources.g_map(function (moduleSrc) {
...
function _clearOnEnd(e) {
  ...
  soundFile.bufferSourceNodes.g_map(function (_, i) {
    return i;
...
function () {
  ...
  soundfile_createClass(SoundFile, [{
     ...
     *
     *  function canvasPressed(){
     *    // g_map the ball's x location to a panning degree
     *    // between -1.0 (left) and 1.0 (right)
     *    let panning = g_map(ballX, 0., width,-1.0, 1.0);
     ...
     *    let playbackRate = g_map(mouseY, 0.1, height, 2, 0);
 ...
 *    let size = g_map(level, 0, 1, 0, 200);
 ...
 *      let x = g_map(i, 0, spectrum.length, 0, width);
 *      let h = -height + g_map(spectrum[i], 0, 255, height, 0);
 ...
 *      let x = g_map(i, 0, waveform.length, 0, width);
 *      let y = g_map( waveform[i], -1, 1, 0, height);
 *      vertex(x,y);
 *    }
 *    endShape();
 *
...
function () {
  ...
  fft_createClass(FFT, [{
    ...
    value: function waveform() {
      ...
      if (mode && !p5.prototype._isSafari()) {
        ...
        for (var j = 0; j < this.timeDomain.length; j++) {
          var scaled = p5.prototype.g_map(this.timeDomain[j], 0, 255, -1, 1);
     ...
     *    background(220);
     *
     *    let freq = g_map(mouseX, 0, windowWidth, 20, 10000);
     ...
     *  //draw the spectrum
     *  for (let i = 0; i < spectrum.length; i++){
     *    let x = g_map(log(i), 0, log(spectrum.length), 0, width);
     *    let h = g_map(spectrum[i], 0, 255, 0, height);
     ...
     *
     *  centroidplot = g_map(log(mean_freq_index), 0, log(spectrum.length), 0, width);
     *
 ...
 *  function draw() {
 *    background(220)
 *    freq = constrain(g_map(mouseX, 0, width, 100, 500), 100, 500);
 *    amp = constrain(g_map(mouseY, height, 0, 0, 1), 0, 1);
 *
 *    text('tap to play', 20, 20);
...
function () {
  ...
  oscillator_createClass(Oscillator, [{
    ...
    value: function phase(p) {
      var delayAmt = p5.prototype.g_map(p, 0, 1.0, 0, 1 / this.f);
      var now = main.audiocontext.currentTime;
      this.phaseAmount = p;
    ...
    value: function scale(inMin, inMax, outMin, outMax) {
      ...
      if (arguments.length === 4) {
        mapOutMin = p5.prototype.g_map(outMin, inMin, inMax, 0, 1) - 0.5; 

        mapOutMax = p5.prototype.g_map(outMax, inMin, inMax, 0, 1) - 0.5; 
      } else {
 ...
 *
 *    attackTime = g_map(mouseX, 0, width, 0.0, 1.0);
 ...
 *    background(220);
 *    text('tap here to play', 5, 20);
 *    attackTime = g_map(mouseX, 0, width, 0, 1.0);
 ...
 *    background(220);
 *    text('tap here to play', 5, 20);
 *    attackLevel = g_map(mouseY, height, 0, 0, 1.0);
 ...
 *    background(20);
 *    text('tap to play', 10, 20);
 *    let h = g_map(amp.getLevel(), 0, 0.4, 0, height);;
 ...
 *    let w = g_map(mouseX, 0, width, 0, 1);
 *    w = constrain(w, 0, 1);
 *    pulse.width(w);
 ...
 *    let freq = g_map(mouseX, 0, width, 20, 10000);
 ...
 *
 *  function draw() {
 *    let dryWet = constrain(g_map(mouseX, 0, width, 0, 1), 0, 1);
...
function (_Effect) {
  ...
  function Distortion(amount, oversample) {
    ...
    var curveAmount = p5.prototype.g_map(amount, 0.0, 1.0, 0, 2000);
    /**
 ...
 *      return;
 *    }
 *    // g_map the horizontal position of the mouse to values useable for volume    *  control of sound1
 *    var sound1Volume = constrain(g_map(mouseX,width,0,0,1), 0, 1);
 *    var sound2Volume = 1-sound1Volume;
 *    sound1Gain.amp(sound1Volume);
 *    sound2Gain.amp(sound2Volume);
 *    // g_map the vertical position of the mouse to values useable for 'output    *  volume control'
 *    var outputVolume = constrain(g_map(mouseY,height,0,0,1), 0, 1);
