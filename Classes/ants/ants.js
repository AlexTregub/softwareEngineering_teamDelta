...
// --- Spawn Ants ---
function Ants_Spawn(numToSpawn, faction = null) {
...
class ant {
  constructor(posX = 0, posY = 0, sizex = 50, sizey = 50, movementSpeed = 1, rotation = 0, img = antImg1,speciesName) {
    ...
    // Initialize state machine with reference to this ant for faction integration
    ...
    this._faction = null; // Will be set to Faction object
  ...
  set faction(value) { 
    ...
    // Handle faction object management
    if (value && typeof value === 'object' && value.addAnt) {
      // Adding to new faction
      if (value !== oldFaction) {
        value.addAnt(this);
      }
    }
  ...
  update() {
    ...
    // Check for automatic faction behaviors
  ...
  checkForEnemies() {
    ...
    for (let i = 0; i < ant_Index; i++) {
      ...
      // Check if different faction and hostile
      if (this._faction && otherAnt.faction && 
          this._faction !== otherAnt.faction) {
        
        const relationship = this._faction.getRelationshipState(otherAnt.faction.name);
        const isHostile = relationship.name === "ENEMIES" || relationship.name === "BLOOD_ENEMIES";
  ...
  // Get nearby ants for faction behavior checking
  ...
  // Handle faction encounters
  encounterAnt(otherAnt) {
    if (!otherAnt || !otherAnt.faction || !this._faction) return null;
